...
const VaultsPage = () => {
  ...
  const handleStake = async (batchId: number, amount: number, duration: number, apy: string, seriesName: string) => {
    if (!publicKey || !program) {
      setNotification({ isOpen: true, title: 'Wallet Required', message: 'Please connect your wallet first to invest in this series.', type: 'error' });
      return;
    }
    ...
    try {
      ...
      const newActivity = { id: Math.random().toString(36).substr(2, 9), type: 'STAKE' as const, amount, date: new Date().toISOString(), status: 'SUCCESS' as const, batchId, duration, seriesName: seriesName || `SS${String(batchId).padStart(3, '0')}` };
      ...
      const certId = `STEAK-CERT-${Math.random().toString(36).substring(2, 10).toUpperCase()}`;
      setCertificateId(certId);
      setLastStakeInfo({ amount, duration, apy, batchId, timestamp: Date.now(), txSig: tx });
      setShowNftModal(true);
      await refetch();
    } catch (error) {
      console.error(error);
      setNotification({ isOpen: true, title: 'Stake Failed', message: error instanceof Error ? error.message : String(error), type: 'error' });
    } finally {
  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 pb-4 border-b-2 border-black border-dashed gap-4">
          ...
          <div className="flex bg-black p-1 rounded-xl border-2 border-black">
            <button
              ...
              className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all ${
                filter === 'active' ? 'bg-grass-primary text-black' : 'text-zinc-500 hover:text-white'
              }`}
            >
              Active
            </button>
            <button
              ...
              className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all ${
                filter === 'finished' ? 'bg-grass-primary text-black' : 'text-zinc-500 hover:text-white'
              }`}
            >
              Finished
            </button>
        ...
        <button onClick={() => refetch()} className="flex items-center gap-2 text-[8px] font-black uppercase text-grass-primary bg-black px-4 py-2 rounded-full hover:scale-105 transition-transform shadow-[3px_3px_0px_0px_rgba(181,255,0,1)] active:shadow-none active:translate-x-[1px] active:translate-y-[1px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 min-h-[400px]">
        {isLoading ? (
          Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-64 bg-zinc-100 border-2 border-black rounded-[32px] animate-pulse flex flex-col p-8 gap-6">
          ...
          filteredBatches.map((batch) => (
            <SukukCard key={batch.batchId} id={batch.id} name={batch.name || batch.id} duration={batch.lockDuration} roi={`${batch.apy} APY`} quota={batch.totalStaked} maxQuota={batch.maxCapacity} isProcessing={isDeploying} isActive={batch.isActive} isHarvested={batch.isHarvested} goats={batch.goats} cows={batch.cows} onStake={(amt) => handleStake(batch.batchId, amt, batch.lockDuration, batch.apy, batch.name)} />
          ))
        ) : (
          <div className="col-span-full flex flex-col items-center justify-center py-20 opacity-30">
            <Filter size={48} className="mb-4" />
            <p className="font-black uppercase text-sm tracking-widest text-center">No pools found in this category.</p>
          </div>
        )}
      </div>

      <CertificateModal isOpen={showNftModal} onClose={() => setShowNftModal(false)} stakeInfo={lastStakeInfo} maturityDate={maturityDate} certificateId={certificateId} />
